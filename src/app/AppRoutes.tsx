...
import type { LockedWalkPresenceMode, NearbyPresence, PresenceUpdatePayload, PresenceUpdateResult, SafeDetourResult, WalkEndResult, WalkPresenceMode } from '../api/walks'
...
import type { MeetPresenceResult, MeetRequest } from '../api/meet'
...
import { clearActiveWalkRoute, clearPendingWalkRoute, clearWalkRoutes, readActiveWalkRoute, writeActiveWalkRoute, writePendingWalkRoute } from '../features/navigation/route-storage'
...
import { dogAge, toAppDog, useAuthProfile } from '../features/profile/useAuthProfile'
import { useCourseRecommendation } from '../features/course-recommendation/useCourseRecommendation'
import { useRepresentativeCourse } from '../features/representative-course/useRepresentativeCourse'
import { useWalkSessionReducer } from '../features/walk-record/useWalkSessionReducer'
...
export function AppRoutes() {
  ...
  } = useAuthProfile(location.pathname, restoredActiveSnapshot?.dogIds)
  const {
    walkPresenceMode,
    setWalkPresenceMode,
    presenceEnabled,
    ...
    setMeetCandidates,
    meetSearchPending,
    setMeetSearchPending,
    meetRequests,
    setMeetRequests,
    meetConnection,
    setMeetConnection,
    selectedWalkRecord,
    setSelectedWalkRecord,
    pendingWalkSelection,
    setPendingWalkSelection,
    activeWalkRoute,
    setActiveWalkRoute,
  } = useWalkSessionReducer(restoredActiveSnapshot)
  const { representativeCourse, representativeCourseDiagnostics } = useRepresentativeCourse(location.pathname)
  const {
    ...
    retryRecommendation,
  } = useCourseRecommendation(location.search)
  ...
  }, [finishMeetSearch, setMeetCandidates, setMeetConnection])
  ...
  }, [setMeetCandidates, setMeetConnection, setMeetRequests])
  ...
  }, [activeWalkRoute, applyMeetResponse, applyPresenceResponse, finishMeetSearch, presenceEnabled, selectedDogIds, setMeetCandidates, setMeetConnection, setNearbyPresence, setPresenceEnabled, walkPresenceMode])
  ...
  }, [restoreTrackedWalk, restoredActiveSnapshot, setBackendWalkStarted, setPresenceEnabled, setWalkApiError, setWalkPresenceMode])
  ...
  }, [applyMeetRequest, applyMeetResponse, backendWalkStarted, isWalkTracking, presenceEnabled, setMeetRequests, walkPresenceMode])
  ...
  useEffect(() => {
    ...
    void walkApi.detail(sessionId)
      ...
      .catch((error: Error) => setWalkApiError(error.message))
  }, [location.pathname, location.search, setSelectedWalkRecord, setWalkApiError])
  ...
  }, [backendWalkStarted, location.pathname, location.search, setWalkBackExitOpen])
  ...
  const runMeetAction = useCallback((action: () => Promise<MeetRequest>) => {
    void action().then(applyMeetRequest).catch((error: Error) => setWalkApiError(error.message))
  }, [applyMeetRequest, setWalkApiError])
  ...
  const acceptMeet = (requestId: string) => runMeetAction(() => meetApi.accept(requestId))
  const rejectMeet = (requestId: string) => runMeetAction(() => meetApi.reject(requestId))
  const cancelMeet = (requestId: string) => runMeetAction(() => meetApi.cancel(requestId))
  const endMeet = (requestId: string) => {
    runMeetAction(() => meetApi.end(requestId))
    setMeetConnection(undefined)
  }
  ...
  }, [presenceEnabled, sendPresenceFix, setMeetCandidates, setMeetSearchPending, walkPresenceMode, walkTracker.currentPosition])
  ...
  const renderWalkSession = (sessionState: 'active' | 'distance-alert' | 'paused') => (
    <>
      <ActiveWalkPage
        ...
        onMeetAccept={acceptMeet}
        onMeetReject={rejectMeet}
        onMeetCancel={cancelMeet}
        onMeetEnd={endMeet}
        onMeetBlock={(id) => {
  ...
  if (location.pathname === '/courses/candidates') {
    if (recommendationRequestId && recommendationLoadError?.requestId === recommendationRequestId && recommendationLoadError.reloadKey === recommendationReloadKey) {
      return <ServerErrorPage onRetry={retryRecommendation} />
    }
